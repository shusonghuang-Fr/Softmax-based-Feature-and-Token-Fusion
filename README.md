...
Firstly, we can run the low-level model 'Step1_feature_extraction.py' for feature extraction, 'Step1_token.py' for token models,  then in the high level, using the pretrained model for token extraction, feature-token fusion, and further using train_step2_test.py
