...
1. Preparing the image dimension. The input image size should be a NumPy array of shape (samples, 224, 224, 3), where 224 * 224 is the number of pixels, and 3 is the number of channels.
2. Feature and token extraction. We can run the low-level model 'Step1_feature_extraction.py' for feature extraction, and run 'Step1_token.py' for token extraction.
3. Feature and token fusion and testing. At higher levels, pretrained models are used for token extraction, feature-token fusion, and then train_step2_test.py is used to test.
